const filter = () => {
  ...
  function showImgItem(curentBtnItem) {
    ...
    setTimeout(() => { // ВИДАЛЕННЯ КЛАСІВ АНІМАЦІЇ ПІСЛЯ ТОГО ЯК ВОНИ ЗАСТОСУВАЛИСЯ (БУВ БАГ ПРИ КЛІКУ, У ЕЛЕМЕНТІВ КОТРІ БУЛИ ПРИСУТНІ В ПОПЕРЕДНЬОМУ ФІЛЬТРІ НЕ ЗАСТОСОВУВАЛАСЬ АНІМАЦІЯ)
      ImgItem.forEach(item => {
        item.classList.remove('animated', 'fadeIn');
      });
