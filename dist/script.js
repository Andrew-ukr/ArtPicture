...
var acardion = function acardion() {
  var header = document.querySelectorAll('.accordion-heading');
  var block = document.querySelectorAll('.accordion-block');
...
/* harmony default export */ __webpack_exports__["default"] = (acardion);
...
var filter = function filter() {
  ...
  function showImgItem(curentBtnItem) {
    ...
    setTimeout(function () {
      // ВИДАЛЕННЯ КЛАСІВ АНІМАЦІЇ ПІСЛЯ ТОГО ЯК ВОНИ ЗАСТОСУВАЛИСЯ (БУВ БАГ ПРИ КЛІКУ, У ЕЛЕМЕНТІВ КОТРІ БУЛИ ПРИСУТНІ В ПОПЕРЕДНЬОМУ ФІЛЬТРІ НЕ ЗАСТОСОВУВАЛАСЬ АНІМАЦІЯ)
      ImgItem.forEach(function (item) {
        item.classList.remove('animated', 'fadeIn');
      });
...
var forms = function forms(a) {
  ...
  var path;
  var imgInput = document.querySelectorAll('input[name="upload"]');
  var fileName;
  imgInput.forEach(function (elem) {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefault();
      var createMassageBlock = document.createElement('div');
      var creaateImgTag = document.createElement('img');
      ...
      fetch(path, {
        ...
        closeStatusMessage();
        item.reset();
        imgInput.forEach(function (element) {
...
var menu = function menu() {
  ...
  var menuBlock = document.querySelector('.burger-menu');
...
var seeMore = function seeMore(btn, cardItem) {
